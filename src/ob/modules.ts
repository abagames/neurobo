...
export class DoInterval extends Module {
  ...
  constructor(actorOrGame: g.Actor | g.Game,
    ...
    super(actorOrGame, modules.length);
  ...
  update() {
    ...
    _.forEach(this.modules, m => {
      m.isEnabled = this.isModulesEnabled;
    });
  }
}

export class DoCond extends Module {
  constructor(actorOrGame: g.Actor | g.Game, public modules: Module[], public cond: Function) {
    super(actorOrGame, modules.length);
  }

  update() {
    const isEnabled = this.cond();
    _.forEach(this.modules, m => {
...
export class JumpOnWall extends Module {
  ...
  update() {
    ...
    if (this.isOnWall) {
      ...
      const avy = g.ui.isPressed ? this.fallSlowVel : this.fallFastVel;
